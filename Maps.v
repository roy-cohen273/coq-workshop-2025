...
Check eqb_spec :
  forall x y : string, reflect (x = y) (eqb x y).
...
(** **** Exercise: 2 stars, standard (shadow_no_unfold)

    The first three lemmas characterize maps: There is no need to
    [unfold] the [t_update] definition to prove other properties.
    Demonstrate this: prove [shadow_no_unfold] without [unfold].
    (if you already proved [t_update_shadow] without [unfold] of
    [t_update], just copy the proof over). *)

Lemma shadow_no_unfold : forall (A : Type) (m : total_map A) x v1 v2,
  (x !-> v2 ; x !-> v1 ; m) = (x !-> v2 ; m).
Proof.
  (* FILL IN HERE *) Admitted.
(** [] *)
...
(** **** Exercise: 3 stars, standard, especially useful (t_update_permute)
    ...
    Using [eqb_spec], prove one final property of
